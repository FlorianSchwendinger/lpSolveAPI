add.constraint <- function(lprec, xt, type = c("<=", "=", ">="), rhs, indices, lhs) {
  if (missing(indices)) {
  ...
  if (!missing(lhs)) {
    range <- abs(rhs - lhs)
    .Call(RlpSolve_set_rh_range, lprec, as.integer(dim(lprec)[1]), as.double(range))
  }
